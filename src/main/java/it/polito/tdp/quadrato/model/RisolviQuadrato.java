...
public class RisolviQuadrato {
    ...
    private void cerca(List<Integer> parziale, int livello) {
        ...
        if (livello % N > 1 && livello != 0) {
            if (!controllaRigaParziale(parziale, livello / N))
                return; // potatura (pruning) dell'albero di ricerca
        }

        // caso intermedio
        for (int valore = N2; valore > 0; valore--) {
    ...
    private boolean controllaRigaParziale(List<Integer> parziale, int riga) {
        int somma = 0;
        for (int col = 0; col < parziale.size() - riga * N; col++)
            somma += parziale.get(riga * N + col);
        return somma < magica;
